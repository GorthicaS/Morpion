...
function game(){
    ...
    afficheur.sendMessage("L'abrutis nommé "+joueurs[tour]+" vas surement jouer quand il auras finis de se toucher");
    ...
    for(var i=0, len = pions.length; i < len; i++) {
        ...
        if(!estValide(this)){
            ...
            afficheur.sendMessage("case occupée abutis !!!<br /> Joueur "+joueurs[tour]+" c'est encore à toi enculer");
        } else{
            ...
            if(jeuEstFini){
                ...
                afficheur.sendMessage("Le joueur "+joueurs[tour]+" a fister l'autre enculer");
           ...
           if(matchNul(pions)) {
            // On affiche un message
            afficheur.sendMessage("Partie terminé ! Vous puez la merde bande d'enculer on dirais le code de joseph, vous êtes inutile!");
           ...
           tour++;
           tour = tour % 2;
           afficheur.sendMessage("Joueur "+joueurs[tour]+" bon au lieu de te toucher la pine tu vas jouer ?!?");
        }
        });
    }
}
